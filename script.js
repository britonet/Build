...
sort.click(function() {
    ...
    for(i=0; i < length; i++) {
        ...
        liObj.color = li.eq(i).attr('class'); //get the class attribute and store as liObj property.
        
        if(liObj.color === choice) { //check if the color for each object matches users choice
    ...
    $.each(items, function(i, value) {    
        ul.append('<li class="'+items[i].color+'">'+items[i].txt+'</li>'); //append list item with correct color class and text.      
    });
